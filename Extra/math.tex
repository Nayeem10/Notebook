...
\subsection{Catalan Numbers, convolution and super}
...
$\displaystyle C_n^{(k)}=\frac{k+1}{n+k+1}{2n+k \choose n}$
$\displaystyle S(m,n)=\frac{(2m)!(2n)!}{m!n!(m+n)!}$
$$\displaystyle C_n^{(k)}=\frac{(2n+k-1)(2n+k)}{n(n+k+1)}C_{n-1}^{(k)}$$
...
$\displaystyle \displaystyle \sum_{i=1}^n \displaystyle\sum_{j=1}^n \gcd(i, j) = \displaystyle \sum_{d=1}^n \phi(d) \lfloor {\frac{n}{d} \rfloor}^2$ = \displaystyle GPS(n-1)+2\cdot\sum_{d|n}\phi(d)\cdot\frac{n}{d}-n \\
