...
class SegmentTree {
 public:
  ...
  int lowerbound(int lo, int hi, int root, node val) {
    ...
    if (leftSum >= val)
      ...
      return lowerbound(mid + 1, hi, rightChild, val - leftSum);
 ...
 public:
  SegmentTree(SegmentTree &st) : tree(st.tree), lazy(st.lazy), n(st.n), merge(st.merge), identity(st.identity), applyUpdate(st.applyUpdate), mergeUpdate(st.mergeUpdate), noUpdate(st.noUpdate) {}

  SegmentTree(vector<node> &input, node (*merge)(node, node), node identity, void (*applyUpdate)(int, int, node &, change), void (*mergeUpdate)(int, int, change &, change), change noUpdate)
      : n(input.size()), merge(merge), identity(identity), applyUpdate(applyUpdate), mergeUpdate(mergeUpdate), noUpdate(noUpdate) {
    tree = new node[n << 2];
    build(input, 0, n - 1);
  ...
  void update(int from, int to, change delta) { update(from, to, 0, n - 1, 0, delta); }
...
void applyUpdate(int lo, int hi, int &val, int delta) { val += delta * (hi - lo + 1); }

void mergeUpdate(int lo, int hi, int &val, int delta) { val += delta; }